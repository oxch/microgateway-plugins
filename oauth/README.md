...
The `oauth` plugin allows you to secure API requests to Apigee Edge Microgateway with either API keys or OAuth 2.0 tokens. Note that, by default, when you include this plugin in the plugins sequence, you can use both API Key validation (requires additional consuming API Application configuration) and OAuth 2.0 validation on API requests. However, you can change the default behavior to allow either API key validation **or** OAuth 2.0  validation. API key only authentication is restricted with this plugin:
* Edge API Application must have a custom `allowAPIKeyOnlyAccess` attribute defined and set to `true` 
* Custom API Application attribute `allowAPIKeyOnlyAccess` has to be included as a part of JWT token, generated by `edgemicro-auth` Edge proxy, as a part of `application_attributes` JSON records array.
...
3. The `oauth` plugin validates either the API Key or the JWT. For API Key only authentication Edge API Application must have a custom `allowAPIKeyOnlyAccess` attribute defined and set to `true`, and, included as a part of JWT token, generated by `edgemicro-auth` Edge proxy, as a part of `application_attributes` JSON records array.
...
Edge Microgateway exchanges the API Key for a JWT when it validates the API key with Apigee Edge. API Key only authentication is restricted with this plugin:
* Edge API Application must have a custom `allowAPIKeyOnlyAccess` attribute defined and set to `true` 
* Custom API Application attribute `allowAPIKeyOnlyAccess` has to be included as a part of JWT token, generated by `edgemicro-auth` Edge proxy, as a part of `application_attributes` JSON records array:
